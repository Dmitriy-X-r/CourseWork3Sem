...
namespace CourseWork3Sem
{
    public partial class FormRegistration : Form
    {
        ...
        private void btnRegistration_Click(object sender, EventArgs e)
        {
            ...
            if(flag == 5)
            {
                Reader newReader = new Reader(fullName, passportID, residentialAddress, phoneNumber, password);
                DB.SaveToDB<Reader>(newReader);
        ...
        private void CheckReader()
        {
            int counter = 0;
            WorkingWithDB DB = new WorkingWithDB();
            string FilePath = DB.GetFileNameForReaders();
            DB.OpenOrCreatFile(FilePath);

            List<Reader> readers = new List<Reader>();
            readers = DB.ReadAllFromDB<Reader>();

            string phoneNumber = "+7" + mtxtPhoneNumber.Text;

            foreach (var reader in readers)
            {
                if (reader.PhoneNumber == phoneNumber)
                {
                    verificationPhoneNumber = false;
                    counter++;
                }
                if (reader.PassportID == mtxtPassportID.Text)
                {
